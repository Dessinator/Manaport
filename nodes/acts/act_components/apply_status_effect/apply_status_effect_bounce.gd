class_name ApplyModifierBounce extends ActComponent

@export var _status_effects: Array[StatusEffectSelection]
@export var _bounce_count: int
...
func consume(act: Act, acting_actor: Actor, receiving_actors: Array) -> Dictionary:
	...
	for i in range(_status_effects.size()):
		...
		for j in range(_bounce_count):
			...
			dict[str(acting_actor.ActorModificationFunction.MODIFY_STATUS) + "_apply_status_effect_" + str(Act.create_modification_id())] = [receiving_actors[rand_actor_index], [duplicate, selection.get_stacks(), selection.does_ignore_bfrt_or_dbrs()]]
	return dict