class_name ModHealthBlast extends ActComponent
...
func consume(act: Act, acting_actor: Actor, receiving_actors: Array) -> Dictionary:
	...
	for i in range(1, receiving_actors.size()):
		if _heal:
			dict[str(acting_actor.ActorModificationFunction.MODIFY_STATUS) + "_heal_" + str(Act.create_modification_id())] = [receiving_actors[i], [residual_damage]]
			continue
		var crit_roll = randf()
		var crit = crit_roll <= crit_chance

		if crit:
			dict[str(acting_actor.ActorModificationFunction.MODIFY_STATUS) + "_damage_" + str(Act.create_modification_id())] = [receiving_actors[i], [residual_damage, crit, int((crit_multiplier * float(residual_damage)) + 0.5)]]
			continue
		dict[str(acting_actor.ActorModificationFunction.MODIFY_STATUS) + "_damage_" + str(Act.create_modification_id())] = [receiving_actors[i], [residual_damage, crit, 0]]
	return dict
